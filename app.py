...
def get_db():
    ...
    conn_str = 'postgresql://postgres:postgres@localhost:5433/FinanceDB'
...
# Define index prices route
@app.route('/api/index_prices')
def get_index_prices_data():
    engine = get_db()
    query = 'SELECT * FROM public."IndexPrices" LIMIT 100;'
    df = pd.read_sql_query(query, engine)
    return jsonify(df.to_dict(orient='records'))
